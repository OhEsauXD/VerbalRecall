...
import type { UserInfo, ToeflListeningTestState } from '@/lib/toeflListeningTestData';
import { INITIAL_LISTENING_TEST_DURATION, TOTAL_LISTENING_PARTS } from '@/lib/toeflListeningTestData';
import { useToast } from '@/hooks/use-toast';

const ToeflListeningStartPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const initialTestState: ToeflListeningTestState = {
      currentAudioPartId: 1,
      answers: [],
      startTime: Date.now(),
      timeRemaining: INITIAL_LISTENING_TEST_DURATION,
      userInfo: finalUserInfo,
      audioPlayCounts: {},
    };

    sessionStorage.setItem('toeflListeningUserInfo', JSON.stringify(finalUserInfo));
    localStorage.setItem('toeflListeningTestState', JSON.stringify(initialTestState));
    router.push('/toefl-listening/section/1');
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-background">
      <Card className="w-full max-w-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center text-primary">Registro Prueba de Comprensión Auditiva TOEFL</CardTitle>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <CardFooter className="p-0 pt-4">
              <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground">
                Comenzar Prueba de Comprensión Auditiva
              </Button>
            </CardFooter>
          </form>
        </CardContent>
...
export default ToeflListeningStartPage;
